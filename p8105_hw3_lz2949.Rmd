...
states_ten = states_ten |>
  filter(unique_locations >= 7)
```
Comments: In 2002, there are `r nrow(states_two)` states observed at 7 or more locations, they are `r print(states_two$state_name)`; in 2010, the number is `r nrow(states_ten)`, they are `r print(states_ten$state_name)`. 
...
```
Comments: The plot has the general trend of lowering average data value over time. 
