...
states_ten = states_ten |>
  filter(unique_locations >= 7)
```
Comments: In 2002, there are `r nrow(states_two)` states observed at 7 or more locations; in 2010, the number is `r nrow(states_ten)`. 
...
```
Comments: In 2010, more responses are collected. In 2010, there are `r nrow(subset(ny_df, year == "2010"))` responses, and in 2006, the number is `r nrow(subset(ny_df, year == "2006"))`. By looking at the plot, poor to excellent responses are evenly distributed among all responses.  
