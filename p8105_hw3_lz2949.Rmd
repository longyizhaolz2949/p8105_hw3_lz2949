...
```
Comments: From the plot, items ordered from resh vegetable and fresh fruit are the most. 
...
states_ten = states_ten |>
  filter(unique_locations >= 7)
```
Comments: In 2002, there are `r nrow(states_two)` states observed at 7 or more locations, they are `r states_two$state_name`; in 2010, the number is `r nrow(states_ten)`, they are `r states_ten$state_name`. 
...
```
Comments: The plot has the general trend of lowering average data value over time. For some states, the fluctuation in data value is really big. Since the plot contains too many data, it is hard to intepret it detailly. 
...
ny_df$year = as.character(ny_df$year)
...
```
Comments: In 2010, more responses are collected. In 2010, there are `r nrow(subset(ny_df, year == "2010"))` responses, and in 2006, the number is `r nrow(subset(ny_df, year == "2006"))`. By looking at the plot, poor has the least data value and very good has the highest data value; for example, in year 2006, "Poor" has a median of `r median(ny_df |> filter(year == "2006", response == "Poor") |>pull(data_value))`, while "Very good" has a median of `r median(ny_df |> filter(year == "2006", response == "Very good") |>pull(data_value))`. 
...
nhanes_df$education = factor(nhanes_df$education, levels = c("Less than high school", "High school equivalent", "More than high school")) # rank education 
...
```{r}
m_nhanes$sum_nhanes = rowSums(m_nhanes[, 6:1445])
...
```
Comments: At high school equivalent and more than high school education levels, males tend to have less daily total activity than females. For example, in the more than high school group, the average total activity for male is `r mean(m_nhanes |> filter(sex == "male", education == "More than high school") |>pull(sum_nhanes))` minutes. For female, the number is `r mean(m_nhanes |> filter(sex == "female", education == "More than high school") |>pull(sum_nhanes))` minutes. Additionally, in all three groups, higher age people tend to have lower total activities. 

```{r}
day_activity = m_nhanes |>
  pivot_longer(cols = min1:min1440, names_to = "activity_minutes", values_to = "activity") 

day_activity$activity_minutes = as.numeric(gsub("min", "", day_activity$activity_minutes)) # remove "min" in the xlabel, and convert it to dbl
...
```
Comments: From these plots, females and males at each education level has similar activity trend in the day. People are most active in the morning and afternoon. From the trend line, females are more active than males when they have education at "high school equivalent" or "more than high school". 
